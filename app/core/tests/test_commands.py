...
from psycopg2 import OperationalError as Psycopg2OpError # This line imports the OperationalError exception from the psycopg2 module and renames it to psycopg2Error. This exception is used to handle database-related errors specific to PostgreSQL.
...
class CommandTests(SimpleTestCase):
    ...
    def test_wait_for_db_ready(self,patched_check):  # The patched_check parameter is the mock object created by the patch decorator.
        ...
        patched_check.assert_called_once_with(databases=['default'])  # The line patched_check.assert_called_once_with(database=['default']) is an assertion in the test that verifies the behavior of the wait_for_db management command. Passing database as an argument.
    ...
    def test_wait_for_db_delay(self,patched_sleep,patched_check):  # order of the arguments must be same as the order of the decorators
        patched_check.side_effect = [Psycopg2OpError]*2 + [OperationalError]*3 + [True]  # returning true at 6th time hoping db will be ready by then
        # explain
